...
struct BCELogitsExpr : public Expr<BCELogitsExpr> {
    DEVICE_HOST float eval(float output, float target) const {
        return fmaxf(output, 0.0f) - output * target + log1pf(expf(-fabsf(output)));
    }
...
struct CrossEntropyExpr : public Expr<CrossEntropyExpr> {
    DEVICE_HOST float eval(float output, float target) const {
        float safe_output = fmaxf(output, 1e-10f);
        return -target * logf(safe_output);
    }
...
namespace loss {
    ...
    struct GradientDivisor {
        DEVICE_HOST static float value(int batch_size, int output_size) {
            return static_cast<float>(batch_size * output_size);
        }
    };

    template <>
    struct GradientDivisor<CrossEntropyLoss> {
        DEVICE_HOST static float value(int batch_size, int output_size) {
            return static_cast<float>(batch_size);
        }
    };

    inline bool applies_activation_derivative(const char* loss_type) {
        if (loss_type == nullptr) return true;
        return strcmp(loss_type, "cross_entropy") != 0 &&
