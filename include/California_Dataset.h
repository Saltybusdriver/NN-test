#ifndef CALIFORNIA_DATASET_H
#define CALIFORNIA_DATASET_H
...
class CaliforniaDataset : public Dataset {
private:
    ...
    inline void load_from_csv(const std::string& csv_path) 
    {
        ...
        if (!file.is_open()) throw std::runtime_error("Could not open California dataset file: " + csv_path);
        std::string line;
        if (!std::getline(file, line)) throw std::runtime_error("Could not read header line");
        features.clear(); targets.clear();
        size_t expected_features = 8;
        while (std::getline(file, line)) 
        {
            std::stringstream ss(line); std::string cell;
            std::vector<float> row_features; float target_value;
            row_features.reserve(expected_features);
            bool row_ok = true;
            ...
            { 
                ...
                try 
                {
                    ...
                    row_features.push_back(std::stof(cell));
                } 
            ...
            if (!row_ok || !std::getline(ss, cell, ',')) continue; // Read target (9th column)
            try 
            {
                if (cell.empty()) continue; // Skip rows with missing target
                target_value = std::stof(cell);
            } 
            catch (const std::exception&) 
            {
                 continue; 
        ...
        if (!features.empty()) num_features_ = features[0].size(); 
        else num_features_ = 0;
        file.close();
    }

    // --- calculate_and_apply_normalization, normalize_features, normalize_targets ---
    // (Identical to BostonDataset, copy them here)
    inline void calculate_and_apply_normalization() 
    {
        if (features.empty()) return;
        size_t n_samples = features.size();
        feature_mean.assign(num_features_, 0.0f); 
        feature_std.assign(num_features_, 0.0f);
...
public:
    explicit inline CaliforniaDataset(const std::string& csv_path, bool normalize = true) 
    {
    ...
    {
        // (Identical logic to BostonDataset constructor)
        if (!features.empty()) num_features_ = features[0].size(); 
        else num_features_ = 0;
        if (features.size() != targets.size()) throw std::runtime_error("Feature/target size mismatch");
        if (apply_norm && !features.empty()) 
        {
            if (train_feature_mean.empty() || train_feature_std.empty()) { calculate_and_apply_normalization(); }
            else 
            {
    ...
    inline ~CaliforniaDataset() override = default;
    ...
    inline void get_sample(size_t index, std::vector<float>& out_features, std::vector<float>& out_targets) const override 
    {
        if (index >= size()) 
        {
            throw std::out_of_range("Index out of range in CaliforniaDataset::get_sample");
        }
    ...
    {
        // California target is a single value
        return 1;
    }
