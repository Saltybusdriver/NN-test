...
NVCC = nvcc
CXX = g++
NVCC_HOST_CXX ?= $(shell command -v g++-15 2>/dev/null)
NVCC_HOST_FLAG := $(if $(NVCC_HOST_CXX),-ccbin $(NVCC_HOST_CXX),)
...
NVCC_BASE_FLAGS := -arch=sm_86 $(NVCC_HOST_FLAG) $(OPENCV_INCLUDE) $(INCLUDE_FLAGS)
...
$(OBJ_DIR)/helpers.o: helpers.cu
	@echo "Compiling CUDA: $< -> $@"
	$(NVCC) $(NVCC_FLAGS) $(CUDA_INC) -DCOMPILE_HELPERS_API -c -o $@ $<

$(OBJ_DIR)/LinearLayers.o $(OBJ_DIR)/Conv2D.o: helpers.cu
