...
bool is_cross_entropy_loss(const char* loss_type)
{
    ...
    int size)
{
    ...
    parallel_for(0, size, [&](int i) 
    {
        element_loss[i] = loss_expr.eval(output[i], target[i]);
    });
}
...
void softmax_cross_entropy_eval_cpu(
    ...
    parallel_for(0, batch_size, [&](int b) 
    {
        ...
        // --- Step 3: Calculate softmax probability and elementwise loss ---
        for (int i = 0; i < output_size; ++i) 
        {
            softmax_prob[i] = std::exp(current_logits[i] - max_logit) * inv_sum;
            current_loss[i] = -current_target[i] * std::log(std::max(softmax_prob[i], 1e-9f));
        }
    });
}
