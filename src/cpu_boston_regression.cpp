    ...
    const std::string& loss_type_str)
{
    ...
    bool use_cuda = false;
    SetUseCuda(use_cuda);
    std::cout << "\n=== Boston Housing Regression Training (CPU) ===\n";
    ...
    std::shared_ptr<BostonDataset> test_dataset;
     try
     {
        test_dataset = std::make_shared<BostonDataset>(test_features_vec, test_targets_vec, true, train_mean_f, train_std_f, train_mean_t, train_std_t);
    }
    ...
    for (int epoch = 0; epoch < num_epochs; ++epoch)
    {
        ...
        while ((batch = train_loader.next_batch()) != nullptr)
        {
            ...
            if (!inputs || !targets)
            {
                 std::cerr << "Warning: Skipping batch with null host data pointers." << std::endl;
                 continue;
            }
        ...
        if (batches_processed_this_epoch > 0)
        {
            ...
            std::cout << "  Train Loss (Avg/Batch): " << avg_epoch_loss << std::endl;
        }
        else
        {
        ...
        double epoch_duration_s = epoch_duration.count();
        epoch_times_s.push_back(epoch_duration_s);

        float current_cpu_mem_epoch = get_peak_cpu_memory_mb();

        std::cout << "  Epoch Time: " << epoch_duration_s << " s" << std::endl;
    ...
    std::cout << "\nTraining complete! CPU Training Time: " << cpu_train_time_ms << " ms\n";
    ...
    // --- Calculate Final Performance Metrics (CPU) ---
    ...
    if (test_samples > 0)
    {
        std::cout << "Test Loss (" << loss_type << "):      " << std::fixed << std::setprecision(6) << final_test_loss << std::endl;
    ...
    // --- Cleanup ---
    std::cout << "\nCleaning up Boston Regression CPU resources...\n";
    for (auto& layer : network.layers)
    {
        delete layer;
    }
    ...
    std::cout << "Total Boston Regression CPU function time: " << total_duration_ms.count() << " ms\n";
    std::cout << "========================================\n";
}
