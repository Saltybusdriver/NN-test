...
// --- Constants specific to CIFAR-10 ---
...
// --- Helper Function to Print Predictions (CPU version) ---
    ...
    int num_classes)
{
    ...
    std::cout << "\n--- Sample CIFAR-10 Predictions (CPU) ---" << std::endl;
    ...
    for (int i = 0; i < num_samples_to_show; ++i)
    {
        ...
        std::cout << std::setw(6) << i << " | "
                  << std::setw(10) << class_names[true_label_idx] << " | "
                  << std::setw(15) << class_names[predicted_label_idx] << " | ";
    ...
    bool use_grayscale)
{
    ...
    std::cout << "\n=== CIFAR-10 Classifier Training (CPU) ===\n";
    ...
    try
    {
        ...
        {
             throw std::runtime_error("One or more CIFAR-10 datasets are empty.");
        }
    ...
    // --- Create DataLoaders ---
    DataLoader train_loader(train_dataset, batch_size, true, 12, 100 * batch_size);
    DataLoader val_loader(val_dataset, batch_size, false, 10, batch_size);
    DataLoader test_loader(test_dataset, batch_size, false, 4, batch_size);
    ...
    try
    {
        ...
        network.add_layer(Conv2d(32, 64, 32, 32, 3, 1, 1, batch_size, "relu"));
        network.add_layer(Flatten(batch_size, 64, 32, 32));
        int flattened_size = 64 * 32 * 32;
    ...
    if (test_batches > 0)
    {
        ...
        std::cout << "Final Test Loss: " << std::fixed << std::setprecision(4) << final_test_loss
                  << ", Final Test Accuracy: " << std::fixed << std::setprecision(4) << final_test_acc << std::endl;
    }
    else
    {
    ...
    if (sample_batch && sample_batch->batch_size > 0)
    {
        ...
        std::vector<float> h_sample_targets(sample_batch->targets_flattened.begin(), sample_batch->targets_flattened.begin() + sample_output_elements);
    ...
    // --- Cleanup ---
    std::cout << "\nCleaning up CIFAR-10 CPU resources...\n";
    for (auto& layer : network.layers)
    {
        delete layer;
    }
    ...
    std::cout << "Total CIFAR-10 CPU function time: " << total_duration_ms.count() << " ms\n";
    std::cout << "CIFAR-10 CPU training finished." << std::endl;
}
