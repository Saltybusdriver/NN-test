    ...
    const std::string& loss_type_str)
{
    ...
    bool use_cuda =true;
    SetUseCuda(use_cuda);
    std::cout << "\n=== Boston Housing Regression Training ===\n";
    ...
    std::shared_ptr<BostonDataset> full_dataset_unnormalized;
    try {
        // Load without applying normalization yet
        full_dataset_unnormalized = std::make_shared<BostonDataset>(dataset_path, false);
    ...
    {
        std::cerr << "Error loading full Boston dataset: " << e.what() << std::endl;
        return;
    }
    ...
    std::shared_ptr<BostonDataset> train_dataset;
    try 
    {

        train_dataset = std::make_shared<BostonDataset>(train_features_vec, train_targets_vec, true);
    } 
    catch (const std::exception& e) 
    {
    ...
    std::shared_ptr<BostonDataset> test_dataset;
    try 
    {
        test_dataset = std::make_shared<BostonDataset>(
            test_features_vec, test_targets_vec, true,
            train_mean_f, train_std_f, train_mean_t, train_std_t
        );
    }
    catch (const std::exception& e) 
    {
    ...
    try 
    {
        network.add_layer(Linear(num_features, 64, batch_size, "relu"));
        network.add_layer(Linear(64, 32, batch_size, "relu"));
        network.add_layer(Linear(32, output_size, batch_size, "none"));
    ...
    if (use_cuda) 
    {
        safeCudaMalloc(&d_element_loss, max_loss_elements * sizeof(float), "d_element_loss (Boston)");
    } 
    else 
    {
    ...
    for (int epoch = 0; epoch < num_epochs; ++epoch) 
    {
        ...
        while ((batch = train_loader.next_batch()) != nullptr) 
        {
            ...
            if (!outputs) 
            {
                 std::cerr << "Error: Network output is null after forward pass." << std::endl;
                 continue;
            }
            ...
            network.compute_loss(outputs, targets, current_loss_buffer_ptr, current_batch_size, loss_type); // <<< Pass current_batch_size for elements >>>
            ...
            network.backward(targets, learning_rate, loss_type);
            ...
            if (batches_processed_this_epoch % 5 == 0 || batches_processed_this_epoch == 1) 
            {
                 std::cout << "  Batch " << std::setw(3) << batches_processed_this_epoch << "/" << total_train_batches_per_epoch
        ...
        {
            float avg_epoch_loss = epoch_loss_sum / batches_processed_this_epoch;
            std::cout << "  Train Loss: " << avg_epoch_loss << std::endl;
        }
        else 
        {
            std::cout << "  No training batches were processed." << std::endl;
        }
    ...
    double avg_epoch_time_s = !epoch_times_s.empty() ? (std::accumulate(epoch_times_s.begin(), epoch_times_s.end(), 0.0) / epoch_times_s.size()) : 0.0; // <<< Use double >>>
    ...
    if (!all_test_targets_denorm.empty()) 
    {
        ...
        for (int i = 0; i < samples_to_show; ++i) 
        {
            ...
            std::cout << std::setw(6) << i << " | "
                      << std::setw(9) << std::fixed << std::setprecision(3) << pred << " | "
                      << std::setw(10) << std::fixed << std::setprecision(3) << actual << " | "
                      << std::setw(10) << std::fixed << std::setprecision(3) << (pred - actual) << std::endl;
        }
        std::cout << "---------------------------------------------" << std::endl;
        std::cout << std::fixed << std::setprecision(6); // Reset precision
    } 
    else 
    {
    ...
    std::cout << "\nCleaning up Boston Regression resources...\n";
    if (use_cuda) 
    {
        safeCudaFree(&d_element_loss, "d_element_loss (Boston)");
    ...
    std::cout << "Total Boston Regression function time: " << total_duration.count() << " ms\n";
    std::cout << "========================================\n";
}
