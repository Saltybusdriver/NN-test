    ...
    int num_samples = 5)
{
    ...
    if(samples_to_show <= 0 || inputs.empty() || outputs.empty()) 
    {
        std::cout << "No samples to display. Exiting.\n";
        return;
    }
...
void train_conv_model(int batch_size = 64, int num_epochs = 5,
    ...
    DataLoader loader(train_dataset, batch_size, false, 12); 
    DataLoader test_loader(test_dataset, batch_size, true, 12);
    ...
    while ((val_batch = test_loader.next_batch()) != nullptr) 
    {
        ...
        first_batch_inputs_h.resize(val_output_elements);
        first_batch_outputs_h.resize(val_output_elements);
        first_batch_actual_size = current_val_batch_size;

        CUDA_CHECK_ERROR(cudaMemcpy(first_batch_inputs_h.data(), d_target, val_output_elements * sizeof(float), cudaMemcpyDeviceToHost));
        CUDA_CHECK_ERROR(cudaMemcpy(first_batch_outputs_h.data(), d_output, val_output_elements * sizeof(float), cudaMemcpyDeviceToHost));
